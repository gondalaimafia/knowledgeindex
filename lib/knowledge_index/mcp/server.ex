defmodule KnowledgeIndex.MCP.Server do
  ...
  def init(port) do
    ...
    # Accept connections in a monitored process so crashes are logged, not fatal
  ...
  @impl GenServer
  def handle_info({:EXIT, _pid, reason}, %{listen_socket: listen_socket} = state) do
    Logger.error("[MCP] Accept loop exited: #{inspect(reason)} — restarting")
    pid = spawn_link(fn -> accept_loop(listen_socket) end)
